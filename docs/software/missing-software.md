...
4. Missing core software or missing SCL?
   - If a [CentOS core software]({{ '/software/core-software.html' | relative_url }}) or a [CentOS Software Collection (SCL)]({{ '/software/scl.html' | relative_url }}) is missing, it is likely that the [the system administrators]({{ '/about/contact.html' | relative_url }}) are willing to add it - please let us know. The basic rules for this is if a package exists in official CentOS Repositories, the EPEL Repository, or the SCL Repositories, then we will install upon request.
