...
Collecting HTSeq
  Using cached https://files.pythonhosted.org/packages/e4/23/aca490d5cf3265dc4b97907231ebb7a4c5b666b4df8eeb815328bef5178b/HTSeq-1.99.2.tar.gz
    ...
    ----------------------------------------
Command "python setup.py egg_info" failed with error code 1 in ~/pip-build-4vgzki6b/HTSeq/
```
...
Installing collected packages: six, distlib, filelock, zipp, typing-extensions, importlib-metadata, platformdirs, importlib-resources, virtualenv
...
created virtual environment CPython3.6.8.final.0-64 in 2251ms
...
Python 2 reached the end of its life on 2020-01-01 in favor of Python 3. At this point, the pip maintainers decided to stop making newer (>= 21.0) versions backward compatible with Python 2, which means we can only upgrade to pip 20.3.\*.

<!-- code-block label="pip2-upgrade" -->
```sh
[alice@{{ site.devel.name }} ~]$ python2 -m pip install --user --upgrade pip=="20.3.*"
