...
<span>⚠️</span> Although rarely needed, if you need to submit your jobs to a specific compute node, which you do via Slurm option `--nodelist`, then you need to also specify `--partition` for that specific node, otherwise `sbatch` gives an error on 'Batch job submission failed: Unspecified error'.
</div>
